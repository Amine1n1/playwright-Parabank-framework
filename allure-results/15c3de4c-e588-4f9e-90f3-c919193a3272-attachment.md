...
- Name: combined/test.combined.spec.ts >> Check Transaction Infos Tests API + UI >> Transfer Funds, use the API to get the TransactionId and Check using UI, if the transaction is available and Infos are correct
- Location: tests/combined/test.combined.spec.ts:17:7
...
```ts
  ...
  14 | 
  15 |     await this.BasePageClick(this.page.getByRole('button', { name: 'Find Transactions' }).first());
  16 |   }
  17 | 
  18 |   async findTransactionByDate(date: string, accountId: string) {
  19 |     await this.BasePageSelectOption('#accountId', accountId);
  20 | 
  21 |     await this.BasePageFill('#transactionDate', date);
  22 | 
  23 |     await this.BasePageClick(this.page.getByRole('button', { name: 'Find Transactions' }).nth(1));
  24 |   }
  25 | 
  26 |   async waitForTableFilled() {
> 27 |     await this.page.waitForSelector('#transactionTable tbody tr td a', {timeout: 20000});
     |                     ^ TimeoutError: page.waitForSelector: Timeout 20000ms exceeded.
  28 |   }
  29 | 
  30 |   async getTransactionDescription(): Promise<string[]> {
  31 |     return this.Locator('#transactionTable tbody tr td').nth(1).allTextContents();
  32 |   }
  33 | 
  34 |   async getTransactionDebit(): Promise<number[]> {
  35 |     const amountStr = await this.Locator('#transactionTable tbody tr td').nth(2).allTextContents();
  36 |     let amountNum = [];
  37 | 
  38 |     for (let i=0; i < amountStr.length; i++) {
  39 |         amountNum[i] = parseFloat(amountStr[i].replace(/[$,]/g, '').trim());
  40 |     }
  41 | 
  42 |     return amountNum;
  43 |     
