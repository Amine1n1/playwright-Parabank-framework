    ...
    - generic [ref=e27]:
      ...
      - generic [ref=e50]:
        ...
        - table [ref=e52]:
          ...
          - rowgroup [ref=e58]:
            - row "38541 $742.75 $742.75" [ref=e59]:
              - cell "38541" [ref=e60]:
                - link "38541" [ref=e61] [cursor=pointer]:
...
```ts
  ...
  29 | 
  30 |     const response = billPayApi.payBill(accountId, amount, newBill);
  31 | 
> 32 |     expect((await response).status).toBe(200);
     |                                     ^ Error: expect(received).toBe(expected) // Object.is equality
  33 | 
  34 |     const responseBody = (await response).json();
  35 | 
  36 |     const balanceAfterPay = await pm.overviewPage.getAccountBalance(accountId);
  37 | 
  38 |     expect(balanceAfterPay).toBeCloseTo(balanceBeforePay - amount, 2);
  39 | 
  40 |   });
  41 | 
  42 | });
