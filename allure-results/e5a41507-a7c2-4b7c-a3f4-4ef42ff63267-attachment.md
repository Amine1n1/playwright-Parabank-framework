...
- Name: api/accounts.api.spec.ts >> Open Account Tests >> Open new Account Successful using API Endpoint createAccount
- Location: tests/api/accounts.api.spec.ts:17:8
    ...
    - generic [ref=e27]:
      ...
      - generic [ref=e50]:
        ...
        - table [ref=e52]:
          ...
          - rowgroup [ref=e58]:
            - row "62739 $742.75 $742.75" [ref=e59]:
              - cell "62739" [ref=e60]:
                - link "62739" [ref=e61] [cursor=pointer]:
                  - /url: activity.htm?id=62739
              - cell "$742.75" [ref=e62]
              - cell "$742.75" [ref=e63]
...
```ts
  ...
  15 |   });
  16 | 
  17 |   test.only('Open new Account Successful using API Endpoint createAccount', async ({ pm, accountsApi }) => {
  18 |     
  ...
  25 | 
  26 |     const accountTypes = [
  27 |       {accountType: 0, accountTpeName: 'CHECKING'},
  28 |       {accountType: 1, accountTpeName: 'SAVINGS'},
  29 |     ];
  30 | 
  31 |     //Use getAccount to get the CustomerId
  32 |     const response = await accountsApi.getAccountById(accounId);
  33 | 
...
> 36 |     const responseBody = await response.json();
     |                          ^ SyntaxError: Unexpected token '<', "<?xml vers"... is not valid JSON
  37 | 
  38 |     const customerId = responseBody.customerId;
  39 | 
  40 |     //open new Account using API Endpoint createAccount
  41 | 
  ...
  47 | 
  48 |     expect(createNewAccountBody.id).toBe(accounId);
  49 |     expect(createNewAccountBody.customerId).toBe(customerId);
  50 |     expect(createNewAccountBody).toContain('balance');
  51 |    
  52 |   });
  53 | 
  54 | });
